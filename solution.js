...
const isEqualDeep = (firstObj, secondObj) => {
  ...
  const compareList = firstObjKeys.map((key) => {
    ...
    if (
      (Number.isNaN(valueOfFirstObject) && Number.isNaN(valueOfSecondObject)) ||
    ...
    if (
      Array.isArray(valueOfFirstObject) &&
      Array.isArray(valueOfSecondObject)
    ) {
      return isArraysEqualDeep(valueOfFirstObject, valueOfSecondObject);
    }
    if (
      typeof valueOfFirstObject === "object" &&
      typeof valueOfSecondObject === "object"
    ) {
